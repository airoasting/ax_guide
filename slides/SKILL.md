...
- **두 블록을 같은 높이로 나누려면 `flex-basis: 0`을 준다.** `flex:1 1 auto`는 콘텐츠가 많은 쪽이 더 커진다. 캔버스 상단(7칸)·하단(3칸) 그리드를 동일 세로 길이로 맞출 때 `.cgrid, .cgrid-bottom { flex-basis: 0 }`로 균등 분할한다.
...
| `.para-*` | 기술 패러다임 타임라인(년도·라인아이콘·브랜드) | 4열, 셀 사이 화살표 없음(삭제) |
| `.car-*` | 커리어 타임라인(시대·카드·주석) | 6열, 화살표 없음 |
| `.cond-impl` | 4조건 아래 "구현 제품 층" 밴드(4C 정렬) | 4열 초록 박스, 위 cond-grid와 동일 칼럼 |
| `.pillar-ico` / `.five-ico` | 카드 상단 라인 아이콘(Feather풍) | 24×24 viewBox, `stroke=currentColor`, accent 그린 |
| `.title-logo` | 제목 행 우측 사례 출처 로고 | `s-titlerow` 안 `<img>`, 하단 정렬 |
| `.curr-dark` | 페이지 31(로드맵) 다크 변형 | `slide--dark` + 내부 요소 색 반전 |
...
## 7. 덱 골격 (현재 36장 · 본문 31 + 부록 5)

표지 → 오픈카톡방(QR) → AI 패러다임 타임라인 → 강정구 커리어 → 전환 메시지(다크) → 목차 → **Q1 트래커** → Part 1 → **Q2 트래커** → Part 2 → **Q3 트래커** → 왜직무 → AX 접근법 4원칙 → 캔버스 한눈에(2장) → T·C·R·A → Harness → 가드레일 → 작성 순서 → 작동 4조건(하단 구현 제품 층) → 커리큘럼 로드맵(다크) → 한 장 요약(다크) → 감사(다크).

- 순서 주의: **AX 접근법 4원칙은 Q3 도입(왜직무) 바로 뒤**, **한 장 요약은 커리큘럼 로드맵 뒤**에 온다(요약이 마지막 본문 슬라이드).
...
- **헤드라인(s-title) 종결**: 명사형 `~함/~임`으로 통일한다(예: "전제가 흔들리고 있음", "이동하고 있음", "접근해야 함"). 합쇼체(`~합니다/~입니다`)로 끝내지 않는다. 이미 명사형인 제목(체언 종결·"~것"·질문형)은 그대로 둔다. 본문·결론 바·리스트는 계속 합쇼체다(헤드라인만 명사형).
- **헤드라인은 메시지 중심으로**: 단순 나열("A하고, B하며, C한다")보다 한 문장으로 핵심 주장을 던진다.
- **eyebrow**: 핵심 한 가지만 짧게. "A · B · C" 식 나열을 피한다.
- **s-meta(우측 상단 라벨)**: 쓰지 않는다(`display: none`). 대신 사례 출처 브랜드는 이 자리에 `.title-logo`(로고 `<img>`, 제목 행 우측·하단 정렬)로 넣는다(예: 10·11쪽 Axios).
- **사례 회사 로고**: 손으로 그린 SVG 대신 **위키미디어(Commons/Wikipedia)의 공식 로고**를 받아 `../assets/logos/`에 저장하고 `<img>`로 쓴다. 비용 카드는 `.costc-name` 안에 로고(높이 22~40px), 카드 높이가 다른 로고로 어긋나지 않게 `.costc-name { min-height; display:flex; align-items:center }`. 정사각형 로고(예: Starbucks 사이렌)는 흰 배경 칩 없이도 라이트 카드 위에 그대로 둔다. 참조하지 않는 로고 파일은 커밋하지 않는다.
...
## 13. 변경 이력 (사용자 수정 요청 반영)

아래는 발표자 검수 과정에서 받은 수정 요청과 반영 결과다. 같은 결정을 다시 내릴 때 참고한다.

### 13.1 문구·종결

- **헤드라인을 명사형 `~함/~임`으로 통일**(합쇼체 9개 변환). 본문·결론·리스트는 합쇼체 유지(§8).
- 11쪽 헤드라인을 메시지 중심으로: "직접 1년을 써보니, AI는 비용 절감뿐 아니라 새로운 사업 기회를 제공 가능함".
- 13쪽 "능력에서 갈림 → 데서 확보 가능함", 16쪽 "이동했음 → 이동하고 있음".
- 요약 헤드라인 "답하는 일입니다 → 답할 수 있어야 합니다", Q2 답 "지어야 → 키워야".
- 27쪽(가드레일·성장) 전체 재작성, 17쪽 함정 5칸 본문 주술 구조 정리, 17쪽 라벨 "P° 1 · 도구부터 논의 → No. 1"(설명어 제거, 함정 이름은 제목에만).
- 22쪽 제목 "시작하는가 → 시작해야 하는가", 캔버스 아이브로 2종 간결화.
- 치환: "챗GPT 계정 → Claude 계정", "도구를 들이는 → 도입하는".
- 결론 바 강조(`<strong>`) 해제. 13쪽 "다음 한 걸음"은 둘째 문장을 덜어 한 줄로.
- 삭제: "다음 한 걸음"(요약), "· AI ROASTING", QR 슬라이드 제목, "SPEAKER · ", "4D의 성실 영역이 적용됩니다", tcra-foot·meta-merge·order 아이브로의 군더더기 문장, 캔버스 축 부제, 진단/참고 칩의 부가 설명.

### 13.2 순서·구조

- **AX 접근법 4원칙을 Q3 도입(왜직무) 바로 뒤로** 이동.
- **한 장 요약을 커리큘럼 로드맵 뒤로** 이동(요약이 마지막 본문).
- 페이지 4 커리어 카드에 로고를 넣었다가 **원래대로 복원**(요청 반영 후 철회 사례).

### 13.3 레이아웃·디자인

- 막대 차트를 세로 축선에 붙임(`.funnel2.axis { padding-left: 0 }`), 바 높이 52→72px.
- 캔버스 축 라벨(WHAT/WHY)을 셀 그룹 중앙에 정렬: 축을 cgrid와 같은 7칸 그리드로 만들고 WHAT=1~3칸, WHY=5~7칸에 배치.
- **캔버스 24·25쪽 상·하단 그리드 세로 길이 통일**(`flex-basis: 0`, §5.2).
- 4조건(30쪽) 하단에 4C 정렬 "구현 제품 층" 밴드(`.cond-impl`: Wiki Brain·Connections·Skill Library·Agent Dashboard) 추가, 이후 셀 위 태그 삭제.
- 20쪽 박스 위 여백 확대, Harness 두 칼럼 항목 시작 높이 정렬, 4조건 라벨을 필 버튼으로.
- **17·18·20쪽 카드에 라인 아이콘**(`.pillar-ico`/`.five-ico`).
- **31쪽 로드맵: 진단·참고 칩 동일 폭**(`.curr-chips` 3열 그리드) + **다크 변형**(`.curr-dark`)으로 30쪽(라이트)과 톤 분리.
- 로드맵 "남기는 자산" 값을 직관적 표현으로, 참고 칩은 제목만, 단계 사이 화살표 삭제.

### 13.4 로고·자산

- 사례 회사 로고를 **위키미디어 공식 SVG**로: Axios(10·11쪽 `.title-logo`), Uber·Microsoft·Starbucks(12쪽 `.costc-name`). `../assets/logos/`에 저장.
- 감사 슬라이드: LinkedIn **아이콘 버튼**(원형, 글자 없음) + 저작권 문구.
- 참조하지 않게 된 로고 파일(페이지 4 복원분)은 커밋 전 삭제.

### 13.5 리더 이해도 9.5 업그레이드 (논리 보강)

비즈니스 리더가 발표로 들을 때의 논리 이해도를 높이려고 더한 것들이다. 페이지는 지우지 않고 추가·보강했다(대규모 수정 전 `slides/ax-keynote.backup-*.html` 백업).

- **Q1 → Q2 다리 슬라이드**(다크 `stmt`): "측정은 시작일 뿐 — 6%를 가른 건 측정 위에 세운 운영체계." 출력 기록(Q1)과 두뇌/운영체계(Q2)의 인과를 잇는다.
- **접근 4원칙 → 캔버스 위계**: appr 슬라이드 하단 `.appr-bridge` 한 줄로 "이 네 접근을 한 장으로 실행하는 도구가 캔버스"라고 명시(병렬 프레임을 위계로).
- **Q3 고도 설정**: 왜직무 qcard에 "리더는 큰 그림만, 작성 규약은 실무 참고" 안내. 좌측 T·C·R·A 슬라이드 eyebrow를 "실무 작성 디테일 · 리더는 개념만"으로 표시해 깊은 구간을 부록처럼 읽게 한다.
- **리더 결정 + 첫 행동 슬라이드**(다크 `.decide`, 요약 뒤): "무엇을 먼저(직무 하나) · 어디에(사람·프로세스) · 누가(책임자 한 명)" + 첫 행동. 의사결정 유용성과 액션 지향성을 보강.
- 결말 흐름: 4조건(라이트) → 로드맵(다크) → 요약(다크) → 리더 결정(다크) → 감사(다크).

### 13.6 Q3 부록 분리 (9.5 달성 레버 실행)

리더 발표 기준 가장 큰 약점이던 Q3 실무 8장 중 **상세 4장을 본문에서 빼 감사 뒤 부록으로 이동**했다(삭제 아님, `ax-keynote.backup-*.html` 백업).

- 본문에 남김(개념): 왜직무 → 캔버스 한눈에 → 작성 규약 두 레이어(오버레이) → 작동 4조건 → 로드맵.
- 부록으로 이동(실무): T·C·R·A · Human/AI Harness · 가드레일·성장 · 작성 순서.
- 부록 진입에 다크 `stmt` 구분 슬라이드("Appendix · 실무 작성 디테일") 추가. 오버레이 슬라이드 eyebrow에 "상세 작성법은 부록" 포인터.
- 결과: 본문 31장으로 리더 호흡에 맞고, 실무 디테일은 필요할 때만 펼친다. 페이지 이동은 `<section>` 블록 단위 splice(파이썬)로 처리.
